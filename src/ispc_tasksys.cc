...
ISPC_USE_TBB). Not all combinations of platform and task system are meaningful.
  If no task system is requested, a reasonable default task system for the
platform is selected.  Here are the task systems that can be selected:
...
machine by assigning one pthread to each hyper-thread, and then uses spinlocks
and atomics for task management.  This model is useful for KNC where tasks can
take over the machine, but less so when there are other tasks that need running
on the machine.
...
*/
      ...
      defined ISPC_USE_OMP || defined ISPC_USE_HPX)
...
#define ISPC_IS_WINDOWS
    ...
    defined(__FreeBSD__)  // pretty much the same for these purposes
...
static void* lAtomicCompareAndSwapPointer(void** v,
                                          void* newValue,
                                          void* oldValue) {
#ifdef ISPC_IS_WINDOWS
...
inline int32_t lAtomicAdd(volatile int32_t* v, int32_t delta) {
#ifdef ISPC_IS_WINDOWS
...
static void lRunTask(void* ti) {
  ...
  taskInfo->func(taskInfo->data, threadIndex, threadCount, taskInfo->taskIndex,
                 taskInfo->taskCount(), taskInfo->taskIndex0(),
                 taskInfo->taskIndex1(), taskInfo->taskIndex2(),
                 taskInfo->taskCount0(), taskInfo->taskCount1(),
                 taskInfo->taskCount2());
}
...
static void* lTaskEntry(void* arg) {
  ...
  while (1) {
    ...
    myTask->func(myTask->data, threadIndex, threadCount, myTask->taskIndex,
                 myTask->taskCount(), myTask->taskIndex0(),
                 myTask->taskIndex1(), myTask->taskIndex2(),
                 myTask->taskCount0(), myTask->taskCount1(),
                 myTask->taskCount2());
    ...
    while (1) {
      if (lAtomicCompareAndSwap32(&lock, 1, 0) == 0) {
        if (threads == nullptr) {
          ...
          srand(time(nullptr));
          for (int i = 0; i < 10; i++) {
            // Some platforms (e.g. FreeBSD) require the name to begin with a
            // slash
            snprintf(name, FILENAME_MAX_LEN, "/ispc_task.%d.%d",
                     static_cast<int>(getpid()), static_cast<int>(rand()));
...
inline void TaskGroup::Sync() {
  ...
  while (numUnfinishedTasks > 0) {
    ...
    myTask->func(myTask->data, 0, 1, myTask->taskIndex, myTask->taskCount(),
                 myTask->taskIndex0(), myTask->taskIndex1(),
                 myTask->taskIndex2(), myTask->taskCount0(),
...
#pragma omp for schedule(runtime)
    for (int i = 0; i < count; i++) {
      ...
      ti->func(ti->data, threadIndex, threadCount, ti->taskIndex,
               ti->taskCount(), ti->taskIndex0(), ti->taskIndex1(),
               ti->taskIndex2(), ti->taskCount0(), ti->taskCount1(),
               ti->taskCount2());
    }
  }
}
...
inline void TaskGroup::Launch(int baseIndex, int count) {
  for (int i = 0; i < count; ++i) {
    ...
    futures.push_back(hpx::async(
        ti->func, ti->data, threadIndex, threadCount, ti->taskIndex,
        ti->taskCount(), ti->taskIndex0(), ti->taskIndex1(), ti->taskIndex2(),
        ti->taskCount0(), ti->taskCount1(), ti->taskCount2()));
  }
}
...
static inline TaskGroup* AllocTaskGroup() {
  for (int i = 0; i < MAX_FREE_TASK_GROUPS; ++i) {
    ...
    if (tg != nullptr) {
      void* ptr = lAtomicCompareAndSwapPointer((void**)(&freeTaskGroups[i]),
                                               nullptr, tg);
      if (ptr != nullptr) {
...
static inline void FreeTaskGroup(TaskGroup* tg) {
  ...
  for (int i = 0; i < MAX_FREE_TASK_GROUPS; ++i) {
    if (freeTaskGroups[i] == nullptr) {
      void* ptr =
          lAtomicCompareAndSwapPointer((void**)&freeTaskGroups[i], tg, nullptr);
      if (ptr == nullptr)
        return;
    }
  }
...
class TaskSys {
 ...
 public:
  ...
  // return old; } inline int inc_end() { int old = end; end =
  // (end+1)%MAX_TASKS; return old; }
